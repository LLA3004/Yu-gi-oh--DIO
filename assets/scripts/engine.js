...
async function createCardImage(randomIdCard, fieldSide) {
    ...
    if (fieldSide === player.player1) {
      // Converte o ID para número antes de enviar
  ...
  async function RemoveAllCardImages() {
    // Seleciona o campo dos inimigos e verifica se ele existe
    let enemyCards = document.querySelector(".card-box.framed#enemy-cards");
    if (enemyCards) {
      let imgElements = enemyCards.querySelectorAll("img");
      imgElements.forEach((img) => img.remove());
    }
  
    // Seleciona o campo dos jogadores e verifica se ele existe
    let playerCards = document.querySelector(".card-box.framed#player-cards");
    if (playerCards) {
      let imgElements = playerCards.querySelectorAll("img");
      imgElements.forEach((img) => img.remove());
    }
  }
